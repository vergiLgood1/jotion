...
const Navigation = () => {
    ...
    const handleMouseMove = (
        ...
        if(sidebarRef.current && navbarRef.current) {
            ...
            navbarRef.current.style.setProperty("width", `calc(100% - ${newWidht}px)`)
        }
    }
  ...
  return (
    ...
    >
        {!!params.documentId ? (
            ...
            onResetWidht={resetWidht}
            />
        ) : (

        <nav className="bg-transparent px-4 py-2 w-full">
            {isCollapsed && <MenuIcon onClick={resetWidht} role="button" className="h-6 w-6 text-muted-foreground"/>}
        </nav>
